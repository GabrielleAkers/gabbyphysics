export const SevenSegmentDisplay = (
    ...
    initial_val: number,
    label: string = "",
    digit_color: string | CanvasGradient | CanvasPattern = "white") => {
    ...
    const displays: [HTMLCanvasElement, CanvasRenderingContext2D][] = [];
    ...
    const draw_segment = (ct: CanvasRenderingContext2D, seg: keyof typeof segments, color: string | CanvasGradient | CanvasPattern = "white") => {
        if (!Object.hasOwn(segments, seg)) return;
    ...
    const draw_segments = (ct: CanvasRenderingContext2D, segs: (keyof typeof segments)[], color: string | CanvasGradient | CanvasPattern = "white") => {
    ...
    const zero_display = (color: string | CanvasGradient | CanvasPattern = "rgba(255, 255, 255, 0.1)") => {
        for (let i = 0; i < num_digits; i++) {
            draw_segments(displays[i][1], ["A", "B", "C", "D", "E", "F"], color);
        }
    };

    const display_num_in_display = (ct: CanvasRenderingContext2D, n: number, color: string | CanvasGradient | CanvasPattern = "white") => {
        if (n >= 10 || n < 0) throw new Error("n must be 0 <= n <= 9");
        let segs: (keyof typeof segments)[] = [];
        let c = color;
        switch (n) {
            case 0:
                segs = ["A", "B", "C", "D", "E", "F"];
    ...
    const pad_n = (from: number, to: number, pad_with: number) => {
        for (let i = from; i < to; i++) {
            display_num_in_display(displays[i][1], pad_with);
        }
    };

    const display_num = (n: number, color: string | CanvasGradient | CanvasPattern = "white") => {
        zero_display();
        const int_n = parseInt(n.toString());
        let str_n = int_n.toString();
        let len_n = str_n.length;
        if (len_n > num_digits) throw new Error(`Not enough digits to display ${n}`);
    ...
    return [display, set_val] as [HTMLDivElement, (n: number) => void];
};
